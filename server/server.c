...
#ifdef __SWITCH__
	if ( unlink( DATABASE_PATH ) == -1 )
	{
		warn( "server: storeDatabase: ERRORE: impossibile cancellare il vecchio database!" );
		return -1;
	}
#endif
...
int main( int argc, char *argv[] )
{
...
#ifdef __SWITCH__
		FILE *db_fp;
		if ( ( db_fp = fopen( DATABASE_PATH, "w" ) ) == NULL )	
#else
		int db_fp;
...
#ifdef __SWITCH__
		FILE* userdb_fp;
		if ( ( userdb_fp = fopen( USERDB_PATH, "w" ) ) == NULL )
#else
		int userdb_fp;
	...
	while ( appletMainLoop() )
	{
		...
		u64 keys = padGetButtonsDown( &gPad );
	...
	while (1)
	{
		...
		tv.tv_sec  = 0;
		tv.tv_usec = 0;
		if ( select( s_list+1, &s_list_fdset, NULL, NULL, &tv ) == -1 )
			warn( "server: Errore nella select" );

		if ( !FD_ISSET( s_list, &s_list_fdset ) )
			continue;
#else
